...
int main(int argc, char** argv) {
    ...
    Morpho morpho_wrapper(i0, i1, j0, j1);
    ...
    CCL ccl_wrapper(i0, i1, j0, j1, p_cca_roi_max1);
    ...
    spu::module::Delayer<uint8_t> delay_RoIs(p_cca_roi_max2 * sizeof(RoI_t), 0);
    spu::module::Delayer<uint32_t> delay_n_RoIs(1, 0);
    ...
    Sigma_delta sd_wrapper((const uint8_t**)IG1, i0, i1, j0, j1, 1, 254, p_sd_n);
    ...
    morpho_wrapper["compute::in_img"] = sd_wrapper["compute::out_img"];
    morpho_wrapper("compute").exec();

    // step 3: connected components labeling (CCL)
    ccl_wrapper["apply::in_img"] = morpho_wrapper["compute::out_img"];
    ccl_wrapper("apply").exec();

    // step 4: connected components analysis (CCA): from image of labels to "regions of interest" (RoIs)          
    f_cca_wrapper["extract::in_labels"] = ccl_wrapper["apply::out_labels"];
    f_cca_wrapper["extract::in_n_RoIs"] = ccl_wrapper["apply::out_n_RoIs_tmp0"];
    f_cca_wrapper("extract").exec();

    // step 5: surface filtering (rm too small and too big RoIs)
    f_filter_wrapper["filter::in_labels"] = ccl_wrapper["apply::out_labels"];
    f_filter_wrapper["filter::in_RoIs"] = f_cca_wrapper["extract::out_RoIs"];
    f_filter_wrapper["filter::in_n_RoIs"] = f_cca_wrapper["extract::out_n_RoIs"];
    f_filter_wrapper("filter").exec();
    delay_RoIs.set_data(f_filter_wrapper["filter::out_RoIs"].get_dataptr<uint8_t>());
    delay_n_RoIs.set_data(f_filter_wrapper["filter::out_n_RoIs"].get_dataptr<uint32_t>());

    delay_RoIs["memorize::in"] = f_filter_wrapper["filter::out_RoIs"];
    delay_n_RoIs["memorize::in"] = f_filter_wrapper["filter::out_n_RoIs"];
    // ----------------------------- //
    // -- Associations (t - 1, t) -- //
    // ----------------------------- //

    // step 6: k-NN matching (RoIs associations)
    knn["match::in_RoIs0"] = delay_RoIs["produce::out"];
    knn["match::in_n_RoIs0"] = delay_n_RoIs["produce::out"];
    ...
    // save frames (CCs)
    if (p_ccl_fra_path) {
        (*log_fra)["write::in_labels"] = f_filter_wrapper["filter::out_labels"];
    ...
    std::vector<vectotask_triplet_t> pip_stages = {
        ...
        // stage 2
        std::make_tuple<vectotask_t, vectotask_t, vectotask_t>(
            { &morpho_wrapper("compute") },
            { &f_filter_wrapper("filter") },
            {  }
        ),
        // stage 3
        std::make_tuple<vectotask_t, vectotask_t, vectotask_t>(
            { &delay_RoIs("produce"), &delay_n_RoIs("produce"), 
                &delay_n_RoIs("memorize"), &delay_RoIs("memorize"), 
                &knn("match"), &tracking_wrapper("perform") },
            {  },
            { /* exception */ }
        )
    };

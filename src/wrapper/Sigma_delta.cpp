...
Sigma_delta::Sigma_delta(const int i0, const int i1, const int j0, const int j1, const uint8_t vmin, const uint8_t vmax) : spu::module::Stateful(), i0(i0), i1(i1), j0(j0), j1(j1), vmin(vmin), vmax(vmax){
	
	const std::string name = "sigma_delta";
	this->set_name(name);
	this->set_short_name(name);
	this->sd_data = sigma_delta_alloc_data(i0, i1, j0, j1, vmin, vmax);
	sigma_delta_init_data(this->sd_data, NULL, this->i0, this->i1, this->j0, this->j1); // laila said check this
	...
	this->create_codelet(t,
		...
		zero_ui8matrix(sd.tmp_img, sd.i0, sd.i1, sd.j0, sd.j1);
		uint8_t **out_img = p[so_img].get_2d_dataptr<uint8_t>();

		sigma_delta_compute(sd.sd_data, in_img, out_img, sd.i0, sd.i1, sd.j0, sd.j1, 1); //VERY IMPORTANT : IDK WHAT N MEANS AND WHAT TO PUT HERE
